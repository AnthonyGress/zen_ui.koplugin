...
<p align="center">適用於 KOReader 的簡潔、極簡 UI。</p>

## 文件
如需最完整及最新的文件，請瀏覽 [https://zen-labs.org/zen-ui](https://zen-labs.org/zen-ui)

## 理念

Zen UI 建基於一個簡單理念：**少即是多。** Zen UI 中所有設計，都是為了移除雜亂或提供清晰價值。介面保持快速、輕巧，並專注於令閱讀更愉快。

在整個開發過程中，有三件事絕不妥協：**效能**、**穩定性**和**易用性**。每項功能都針對電池效率及反應速度作出調校。

## 速度與效能

Zen UI 設計輕巧而高效。即使書庫包含數千本書，速度、反應能力或資源使用量都不會有明顯變化。修補程式會策略性注入，並只在需要時載入。不論書庫規模如何，Zen UI 都能維持一致效能，而不會過度消耗裝置的電池或記憶體。
...
- 簡潔的馬賽克及列表檢視選項，可盡量放大書籍封面，並提供多種選項
- 用於資料夾縮圖的書籍封面圖庫
- 可設定排序、每頁項目數，以及橫向/直向版面
- 檔案瀏覽器中的精簡內容選單。點按並按住即可快速存取詳細資料、全螢幕封面圖、閱讀狀態等。
...
### 底部導覽列
位於書庫底部的簡潔分頁式導覽列。可設定分頁（書庫、漫畫、我的最愛、作者、歷史、收藏集等），支援選用標籤文字、自訂圖示及可排序版面。

<img src="./images/quickstart/onboarding/navbar.png" width="500" alt="導覽列">

### Zen 模式

將預設 KOReader 介面精簡至最必要部分。- 隱藏 KOReader 的所有預設選單，只保留單一統一的 Zen UI 設定分頁。

<img src="./images/quickstart/onboarding/zen_mode.png" width="175" alt="Zen 模式">

### 鎖定模式

為無干擾閱讀建立一個更受限制的沙盒。鎖定模式旨在讓裝置專注於核心流程：瀏覽書籍及閱讀書籍。對不應被任何設定或不必要選項造成負擔的年長或年幼讀者，這個模式非常適合。
...
### 自訂狀態列
閱讀器中提供極簡狀態列，書庫中提供更詳細的狀態列。只顯示你想要的內容：時間、電量、磁碟空間、自訂文字——全部均可選用，並可獨立開關。

<img src="./images/quickstart/onboarding/status_bar.png" width="500" alt="狀態列">

### 閱讀器改進
- 自訂頁面瀏覽器（類似 Kindle）可用於快速瀏覽頁面、搜尋書籍、快速更改字型大小。
- 停用底部選單，避免意外更改字型大小。
- 邊緣觸控防護，可避免握住裝置觸控螢幕邊緣時誤觸手勢。
- 從少量預先設計的閱讀進度列中選擇，或建立並儲存自己的進度列。點按一下即可切換預設。
...
每個排程都可單獨或配合使用。這種細緻方式可讓你按偏好精準調整燈光。

### OPDS 插件主題
OPDS 插件會遵循你的所有 Zen UI 書庫樣式設定——在本機書庫與網上目錄之間建立一致的視覺體驗。

- 來自書庫設定的馬賽克、封面格狀及列表檢視模式
- 如已啟用，則對封面套用圓角
- 可從快速設定一鍵存取的預設目錄

使用你為本機收藏自訂的同樣簡潔、一致介面，瀏覽喜愛的 OPDS 來源。

## 統一設定
- 將最重要的設定整合到單一、更精簡的設定分頁
- 設定按功能區域分組（書庫、控制、啟動器、閱讀器、附加功能、關於）。
- 大多數功能都可獨立開關，並已選擇一些合理的預設值。
- 無需離開 KOReader 或連接電腦，即可直接從設定更新 Zen UI。
...
定位。在 Zen UI 之前載入的插件，應在收到 `ZenUIReady` 時註冊；使用
`_G.__ZEN_UI_UNREGISTER_HOME_ITEM(id)` 取消註冊。

如引數無效或與內建 ID 衝突，註冊會回傳 `false`。
註冊既有外部 ID 會取代其建構器及選項。

## 前置要求

- 必須先安裝 KOReader 才能使用 Zen UI。[安裝 KOReader](https://github.com/koreader/koreader#installation)
- 停用或解除安裝任何會修改 UI 的**其他插件/修補程式**，例如 Simple UI、Project: Title、VOS，因為它們可能發生衝突並造成不穩定。


## 安裝

1. 前往 [Releases](https://github.com/AnthonyGress/zen_ui.koplugin/releases) 頁面，並從最新版本下載 `zen_ui.koplugin.zip`。
2. 解壓縮封存檔。你應會得到名為 `zen_ui.koplugin` 的**資料夾**。
3. 將 `zen_ui.koplugin` **資料夾**複製到你裝置上的 KOReader 插件目錄：請見下表
      - 請確認複製的是已解壓縮的**資料夾**，而**不是 .zip** 檔案本身
4. 重新啟動 KOReader。Zen UI 會自動載入
      - 如果你沒有看到 Zen UI 載入，請在 Tools > More tools > Plugin management > Zen UI 中手動啟用插件
...
如果你之前使用過 [Project Title](https://github.com/joshuacant/ProjectTitle)，在使用 Zen UI 前必須停用或移除它。兩個插件都會修補 Cover Browser，同時啟用會造成衝突。

請選擇以下其中一種方式：

- **移除它** — 從你的 KOReader 插件目錄刪除 `projecttitle.koplugin` 資料夾。
...
若要貢獻翻譯或修正現有翻譯，請參閱 [locales/README.md](locales/README.md) 及 [CONTRIBUTING.md](CONTRIBUTING.md)。

## 鳴謝

Zen UI 是原創作品，但如果沒有更廣大的 KOReader 社群，它不會存在。多個開源項目提供了元件、靈感、參考實作，或經改寫並延伸建構的程式碼：

- **[joshuacant/ProjectTitle](https://github.com/joshuacant/ProjectTitle)** — 對我來說，一切由這個插件開始。這是我第一次接觸 KOReader 插件與替代 UI。
- **[qewer33/koreader-patches](https://github.com/qewer33/koreader-patches)** — 底部導覽列與快速設定元件。也提供了額外的修補方式及想法，特別是 UI 自訂相關。
- **[sebdelsol/KOReader.patches](https://github.com/sebdelsol/KOReader.patches)** — 啟發 Zen UI 多項功能的修補程式與 UI 技術。
- **[doctorhetfield-cmd/simpleui.koplugin](https://github.com/doctorhetfield-cmd/simpleui.koplugin)** — 另一個 KOReader UI 插件，既是靈感來源，也提供了如何在整個插件套用語言翻譯的範例。
...
- **審查 AI 產生的程式碼** - 如果使用 AI 工具，所有程式碼都必須在提交前徹底審查及測試（這本來就應該做，但對 AI 產生的程式碼尤其如此）
- **維持一致性** - 新程式碼必須符合項目既有的風格、主題及整體使用者體驗
...
請參閱 [SECURITY.md](SECURITY.md) 了解如何回報漏洞。
