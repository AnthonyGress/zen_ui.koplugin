...
Controls adds a fast control panel to KOReader. It allows you to toggle Wi-Fi, frontlight, rotation, Zen UI modes, brightness, and warmth controls with a single swipe. You can also add action and plugin buttons for quick access.
...
- Get an automatically suggested icon when creating an action or plugin button.
...
| Action or plugin button > Show | Shows or hides the button in Controls. |
| Action or plugin button > Delete | Deletes the button and removes it from the order list. |
...
| Reset to defaults | Restores the default Controls button layout. Action and plugin buttons are not removed; they are disabled and kept in your saved configuration. |
